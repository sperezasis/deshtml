...
- [ ] **DESIGN-01**: Generated HTML uses only colors from the Caseproof Documentation System palette — no hex literals appear outside the `:root` CSS variable block
- [ ] **DESIGN-02**: Generated HTML uses only typography rules from the Caseproof Documentation System (Inter via Google Fonts, with `-apple-system, sans-serif` fallback)
- [ ] **DESIGN-03**: Generated HTML uses only components from a closed component library — no freelance markup
...
- [ ] **DESIGN-05**: Design tokens (palette, typography, components, format skeletons) ship as verbatim CSS / HTML files in `skill/design/` that Claude pastes — never paraphrases — into output
...
- [ ] **DESIGN-07**: Generated HTML declares `color-scheme: light` and includes the meta tag that prevents browser forced-dark-mode from inverting the palette
